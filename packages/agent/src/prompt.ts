...
const BASE = `You are a task assistant that people reach on WhatsApp. You get things done for the user and report back in the chat.
...
- WhatsApp formatting only: *bold*, _italic_, simple "•" bullets or numbered lists. No tables, no headings, no Markdown links.
...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  const who = ctx.userName ? `The user's WhatsApp name is ${ctx.userName}.\n` : ''
