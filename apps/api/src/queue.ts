...
import { QUEUES } from '@wa/core'
import type { WebhookEvent } from '@wa/whatsapp'

const DEDUP_WINDOW_MS = 3 * 24 * 60 * 60_000

/**
 * Delivery is at-least-once. The job id is derived from the WhatsApp id, and
 ...
 */
export function jobIdFor(event: WebhookEvent): string {
  const key = event.kind === 'message' ? `m:${event.message.id}` : `s:${event.status.id}:${event.status.status}`
...
export function createInboundQueue(redisUrl: string) {
  ...
  const queue = new Queue<WebhookEvent>(QUEUES.inbound, {
    connection,
    defaultJobOptions: {
      attempts: 5,
  ...
  return {
    async enqueue(events: WebhookEvent[]) {
