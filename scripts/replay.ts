...
const [{ default: Anthropic }, core, db, tools, wa, inbound] = await Promise.all([
...
if (!verifySignature(Buffer.from(raw), signature, config.WHATSAPP_APP_SECRET)) throw new Error('signature mismatch')
const { events, skipped } = wa.parseWebhook(JSON.parse(raw))
...
const handle = inbound.createInboundHandler({
  repo,
  client: fake,
  sender: wa.createSender({ client: fake, getLastInboundAt: (waId) => repo.getLastInboundAt(waId) }),
...
for (const call of fake.calls) process.stdout.write(`${JSON.stringify(call, null, 2)}\n`)
